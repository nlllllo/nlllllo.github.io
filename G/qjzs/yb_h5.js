...
function gethtml5location_fun() {
	try {
		if (browser.versions.android) {
			...
			document.getElementById("showCityPicker").style.display = "inline-block";
			document.getElementById("address").textContent = "无法获取您的地理位置，请确保开启手机定位和易班APP位置权限";
		}
	} catch (e) {

		document.getElementById("showCityPicker").style.display = "inline-block";
		document.getElementById("address").textContent = "无法获取您的地理位置，请确保开启手机定位和易班APP位置权限";
		//alert('无法获取您的地理位置，请确保手机开启位置权限和GPS定位')
...
function yibanhtml5location(postion) {
	...
	try {
		var p = (eval('(' + postion + ')'));
		var editedHTML = document.getElementById("address");
		editedHTML.textContent = p.address;
		document.getElementById("showCityPicker").style.display = "none";
		...
		//alert(postion);
		document.getElementById("showCityPicker").style.display = "inline-block";
		document.getElementById("address").textContent = "无法获取您的地理位置，请确保开启手机定位和易班APP位置权限：" + postion;
		if (postion.indexOf("定位失败") > -1 && postion.indexOf("打开") == -1) {
			//document.getElementById("showCityPicker").style.display = "inline-block";
		}
	}
}
...
function getScanResult(info) {
	var s = info.replace("yiban_scan_result", "");
	//window.location.href = "teacher-backschool.html?studentID=" + s;
	//alert(info)
	//document.getElementById("returnValue").value = info;

	setTimeout(function() {
		showStudentBackSchoolInfo(s);
	}, 500);
}
